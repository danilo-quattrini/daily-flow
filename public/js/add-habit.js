...
function addHabitToList(habit) {
    ...
    if (habitItem) {
        ...
        habitItem.querySelector('.habit-time').textContent = `Started At ${habit.time}`;
        habitItem.querySelector('.habit-status-text').textContent = habit.progress === 100
            ? 'Status: Done'
            : habit.progress > 0
            ? 'Status: In Progress'
            : 'Status: Not done';
        ...
        habitItem.querySelector('.habit-delete-button').addEventListener('click', () => deleteHabit(habit.id));
        habitItem.querySelector('.habit-edit-button').addEventListener('click', () => openEditPopup(habit));
    }
}
