...
exports.getHabit = async (req, res) => {
  ...
  const sql =
    "SELECT habit_id AS id, habit_name AS name, start_date AS time, progress FROM habits WHERE user_id = ? ORDER BY created_at DESC";
...
exports.addHabit = async (req, res) => {
  ...
  const sql =
    "INSERT INTO habits (user_id, habit_name, frequency, start_date, progress, created_at) VALUES (?, ?, ?, ?, ?, ?)";
  db.query(
    sql,
    [userId, name, frequency, startDate, progress, createdAT],
    (err, result) => {
      if (err) {
        console.error(err);
...
exports.updateHabit = async (req, res) => {
  ...
  const sql =
    "UPDATE habits SET habit_name = ?, frequency = ?, start_date = ?, progress = ? WHERE habit_id = ? AND user_id = ?";
  db.query(
    sql,
    [name, frequency, startDate, progress, habitId, userId],
    (err, result) => {
      if (err) {
